...
請回傳以下 JSON 格式（不要加 markdown 標記，直接回傳純 JSON）：
{
    ...
    "paragraphs": [
        {
            "speaker": "說話者名稱（如果是對談/訪談格式，例如：芹澤、MOTSU、――。如果不是對談格式則留空字串）",
            "lines": [
                {
                    "original": "該段落中的一個句子（日文）",
                    "translation": "該句的中文翻譯"
                }
            ]
        }
...
重要要求：
- paragraphs 必須按照原文的段落結構分組，每個段落是一個物件
- 如果是訪談/對談格式，用 speaker 標示說話者（如：芹澤、MOTSU、――表示提問者）
- 每個段落內的 lines 必須包含該段落的【每一個句子】，逐句翻譯，不可省略
- 普通文章的 speaker 留空字串即可
- 單字至少提取 8-15 個重要單字
- 文法至少提取 3-5 個文法點
- 重點例句至少 3-5 句
- 所有翻譯要自然通順
